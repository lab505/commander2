...
class Mission_Widget_Item(PyQt5.QtWidgets.QTreeWidgetItem):#飞行区域的列表的创建
    def __init__(self, parent, rc, type_, area_name, binding_object):
        ...
        self.fly_location =[]
        self.get_fly_location('aircraft/fly_data.txt')
        '''
    ...
    '''
    def get_fly_location(self, file_name):
        ...
        with open(file_name, 'r') as file:
            for line in file:
                ...
                if(len(line_split)==4):
                    self.fly_location.append((float(line_split[0]),float(line_split[1]),float(line_split[2]),float(line_split[3])))
